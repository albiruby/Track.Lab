...
import {
  ExportPanel,
  ResultActionRow,
} from "@/components/calculator/CalculatorSystem";
...
export default function HeartRateLabPage() {
  ...
  const handleCalculate = (e?: React.FormEvent) => {
    ...
    if (calcType === "max") {
      ...
      res = {
          ...
          <div className="flex flex-col gap-3">
            {hasMeasured ? (
              <div className="flex justify-between items-center p-3 border-2 border-border-heavy bg-primary text-primary-foreground rounded-lg shadow-[2px_2px_0px_rgba(23,23,23,1)]">
                <div className="flex flex-col">
                  <span className="font-bold text-xs uppercase tracking-wider text-primary-foreground/80">
                    User Measured
                  </span>
                  <span className="text-[10px] opacity-80">
                    True max if measured correctly
                  </span>
                </div>
                <span className="font-mono font-bold text-2xl">
                  {hasMeasured} <span className="text-sm">bpm</span>
                </span>
              </div>
            ) : null}
            ...
            <div className="flex flex-col border-2 border-border-heavy rounded-lg overflow-hidden bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
              {methods.map((m, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center p-2 border-b-2 border-border-heavy last:border-0 hover:bg-muted/50 transition-colors"
                >
                  <div className="flex flex-col">
            ...
            <ResultActionRow
              actions={[{ label: "Zone Lab", href: "/zone" }]}
            />
          </div>
        ),
        ...
        limitations:
          "Measured > test > estimate. Population averages lose individual accuracy.",
      };
      ...
      if (!max && !lthrVal && !rest)
        return setError("Please provide HR limits to calculate zones.");
      if (max > 0 && rest > 0 && max <= rest)
        return setError("Max HR must be greater than Rest HR.");
      ...
      res = {
              ...
              <div className="flex flex-col md:flex-row gap-2 border-2 border-border-heavy p-3 rounded-lg bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
                <div className="flex flex-col flex-1">
                  <span className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground">
                    Standard 5-Zone (%HRmax)
                  </span>
                  <span className="text-xs bg-muted px-1.5 py-0.5 rounded mt-1 mb-1 font-mono">
                    {Math.round(max * 0.5)}-{Math.round(max * 0.6)} •{" "}
                    {Math.round(max * 0.6)}-{Math.round(max * 0.7)} •{" "}
                    {Math.round(max * 0.7)}-{Math.round(max * 0.8)} •{" "}
                    {Math.round(max * 0.8)}-{Math.round(max * 0.9)} •{" "}
                    {Math.round(max * 0.9)}+
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-2 border-2 border-border-heavy p-3 rounded-lg bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
                <div className="flex flex-col flex-1">
                  ...
                  <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded mt-1 mb-1 font-mono">
                    Z1: {Math.round(rest + hrr * 0.5)}-
                    {Math.round(rest + hrr * 0.6)} | Z2:{" "}
                    {Math.round(rest + hrr * 0.6)}-
                    {Math.round(rest + hrr * 0.7)}
                  </span>
                </div>
              </div>
            )}
                ...
                {lthrVal > 0 && (
                  <div className="flex flex-col md:flex-row gap-2 border-2 border-border-heavy p-3 rounded-lg bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
                    <div className="flex flex-col flex-1">
                      ...
                      <span className="font-mono text-sm mt-1">
                        {Math.round(lthrVal * 0.85)} -{" "}
                        {Math.round(lthrVal * 0.89)} bpm
                      </span>
                    </div>
                  </div>
                )}
        ...
        limitations:
          "Calculates mathematically exact ranges; physiological borders naturally vary.",
      };
      ...
      res = {
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 border-2 border-border-heavy p-4 rounded-xl bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
              ...
              <div className="flex items-end gap-2">
                <span
                  className={`font-mono text-3xl font-black ${dPct > 0 ? "text-destructive" : "text-primary"}`}
                >
                  {dPct > 0 ? "+" : ""}
                  {dPct.toFixed(1)}%
                </span>
              ...
              {dPct > 5 && (
                <span className="text-[10px] font-bold text-destructive">
                  Drift &gt; 5% suggests fatigue or cardiac drift.
                </span>
              )}
            </div>
            ...
            {decoupling !== null && (
              <div className="flex flex-col gap-1 border-2 border-border-heavy p-4 rounded-xl bg-primary text-primary-foreground shadow-[2px_2px_0px_rgba(23,23,23,1)]">
                ...
                <span className="text-[10px] font-bold text-primary-foreground/80">
                  Pace-to-HR Efficiency Change
                </span>
              </div>
            )}
            <div className="mt-2 flex gap-2">
              <ResultActionRow actions={[{ label: "RPE Lab", href: "/rpe" }]} />
            </div>
          </div>
        ),
        ...
        limitations:
          "Input-based durability estimate, not diagnosis. Ignores heat/terrain.",
      };
      ...
      if (!p || !r1)
        return setError("Peak HR and 1-min Recovery are required.");
        ...
        result: (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 border-2 border-border-heavy p-4 rounded-xl bg-card shadow-[1px_1px_0px_rgba(23,23,23,1)]">
              ...
              {rec1 < 12 ? (
                <span className="text-[10px] font-bold text-destructive">
                  Drop &lt; 12 bpm. Monitoring recommended.
                </span>
              ) : (
                <span className="text-[10px] font-bold text-emerald-600">
                  Healthy immediate recovery phase.
                </span>
              )}
            </div>
  ...
  return (
    <CalculatorPageShell
      ...
      subtitle="Calculate maximum heart rate, preliminary zones, drift, and recovery patterns."
    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {calcType === "max" && (
              <>
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor="measuredMax">
                    Measured Max HR (optional)
                  </Label>
                  <Input
                    id="measuredMax"
                    type="number"
                    ...
                    placeholder="e.g. 192"
                    value={measuredMax}
                    onChange={(e) => setMeasuredMax(e.target.value)}
                  />
                </div>
              </>
            )}
            ...
            {calcType === "zones" && (
              <>
                <div className="sm:col-span-2">
                  <Label htmlFor="ageZ">Age (for MAF)</Label>
                  <Input
                    id="ageZ"
                    type="number"
                    value={age}
            ...
            {calcType === "drift" && (
              <>
                <div>
                  <Label htmlFor="hr1">First half HR Avg</Label>
                  <Input
                    id="hr1"
                    type="number"
                    value={hr1}
                ...
                <div>
                  <Label htmlFor="hr2">Second half HR Avg</Label>
                  <Input
                    id="hr2"
                    type="number"
                    value={hr2}
                ...
                <div className="sm:col-span-2 text-xs text-muted-foreground font-bold mt-2">
                  Optional (for Aerobic Decoupling)
                </div>
                <div>
                  <Label htmlFor="pace1">First half Pace (MM:SS)</Label>
                  <Input
                    id="pace1"
                    type="text"
                    placeholder="5:30"
                    value={pace1}
                    onChange={(e) => setPace1(e.target.value)}
                  />
                </div>
                <div>
                  <Label htmlFor="pace2">Second half Pace (MM:SS)</Label>
                  <Input
                    id="pace2"
                    type="text"
                    placeholder="5:45"
                    value={pace2}
                    onChange={(e) => setPace2(e.target.value)}
                  />
                </div>
              </>
            )}
            ...
            {calcType === "recovery" && (
              <>
                <div className="sm:col-span-2">
                  <Label htmlFor="peakHr">Peak HR (End of interval)</Label>
                  <Input
                    id="peakHr"
                    type="number"
                    value={peakHr}
                ...
                <div>
                  <Label htmlFor="rec1Hr">HR after 1 minute</Label>
                  <Input
                    id="rec1Hr"
                    type="number"
                    value={rec1Hr}
                    onChange={(e) => setRec1Hr(e.target.value)}
                    required
                  />
                </div>
                <div>
                  <Label htmlFor="rec2Hr">HR after 2 minutes</Label>
                  <Input
                    id="rec2Hr"
                    type="number"
                    value={rec2Hr}
                    onChange={(e) => setRec2Hr(e.target.value)}
                  />
                </div>
              </>
            )}
        ...
        <div className="flex flex-col gap-4 h-full">
          {result ? (
            <>
              ...
              <ExportPanel
                textToCopy={resultToText(result, "Heart Rate Lab Result")}
              />
            </>
          ) : (
